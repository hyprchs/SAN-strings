...
for i, san_string in enumerate(san_strings):
    ...
    if san_string.startswith('O-O-O'):
        words.append(('castles', 'castles queenside'))
    elif san_string.startswith('O-O'):
        words.append(('castles', 'castles kingside'))
    else:
        idx = 0
        while idx < len(san_string):
            ...
            if c.isupper():
                ...
                file = san_string[idx + 1]
                rank = san_string[idx + 2]
    ...
    if all(isinstance(w, str) for w in words):
        spoken_san_strings.add(' '.join(words))  # type: ignore
        continue
